...
**Documento vivo.** Registra todo lo evaluado para el laboratorio: lo construido, lo aprobado, lo en exploración y lo rechazado, con su porqué. Crece con cada candidato nuevo; ninguna entrada se borra (un rechazo documentado también enseña). Última actualización: 28 de julio de 2026.
...
**Nota de vecindario (tanda 7).** El territorio "matemática de la secuencia del Rey Wen" se activó en 2026, y conviene registrar los trabajos hermanos con sus fechas para no reinventar ni chocar sin querer: Chan (2026, arXiv:2604.09234), análisis estadístico del Rey Wen contra barajados libres (diálogo en el exp. 19); Radisic (2026, arXiv:2601.07175), emparejamientos óptimos equivariantes del 6-cubo con una prueba formalizada en Lean, cuya partición de los pares coincide con la nuestra (diálogo en el exp. 43); e iching-math (recurso web) con el tipo de ciclo de la permutación del Rey Wen [52, 10, 2], que reprodujimos y coincide con el del exp. 09 (orden 260 = mcm de 52, 10 y 2). McKenna (Timewave, años 80) se nombra como antecedente histórico pero queda fuera del criterio (sección 12 del propio registro por analogía). Ninguno de estos usa el enfoque de la hermandad (estadística de permutaciones ENTRE los órdenes históricos), que es el sello quinto.

**Sellos de hallazgo propio (tanda 5, búsqueda 2026-07-25, en docs/informe-originalidad.md).** El conteo de sellos pasa de 1 a 4: se conceden a espectro-walsh (el espectro de Walsh de la ordenación del Rey Wen y su 77,4% en órdenes pares), comparador-particiones (el ARI = −0,125 entre palacios y cosets) y permutacion (el empate 1013/1008/1008 y los costos en líneas); ninguno aparece publicado. Nota (tanda 6): el reencuadre del 1008 refuerza el sello de permutacion sin tocar su busquedaNota, que es documentación histórica de la búsqueda; el 1008 es la esperanza exacta del azar (n(n-1)/4, desviación 86,3), así que el empate significa que los tres órdenes están a la distancia esperada del binario (ver hermandad-ordenes). **Sello quinto (tanda 7, búsqueda 2026-07-27):** hermandad-ordenes (la estadística de permutaciones ENTRE los órdenes históricos, el 759 de Rey Wen-Mawangdui con z = −2,89; ningún trabajo publicado la hace, ver la nota de vecindario). El conteo pasa de 4 a 5. El sello de pregunta-del-par queda reevaluable tras la ampliación del diálogo con Radisic (2026): la partición de los pares que formaliza el preprint coincide con la nuestra, así que un sello futuro deberá acotarse a lo que no esté en su marco. El sello del test de aleatoriedad del Rey Wen (rey-wen-aleatorio) queda DENEGADO POR AHORA: Chan (2026), *Statistical properties of the King Wen sequence* (arXiv:2604.09234), es antecedente directo con cuatro propiedades significativas contra barajados libres. En su lugar se publica el diálogo condicional del experimento 19 (tres de esas cuatro propiedades son corolarios de la regla de pares; la cuarta, marginal), con ficha [chan2026]. Sello reevaluable en una búsqueda futura.
...
| Entrada | Estado | ¿Aplica? | Por qué |
|---|---|---|---|
| Hermandad de los órdenes (inversiones de Kendall) | 🟢 hermandad-ordenes · SELLO (tanda 7) | Sí | La esperanza de inversiones entre órdenes aleatorios es n(n-1)/4 = 1008 (desviación 86,3, teorema mahoniano verificado contra Monte Carlo); entre sí, solo Rey Wen-Mawangdui se aparta del azar (759, z = −2,89, p Monte Carlo 0,003, sobrevive Bonferroni). Ampliación (tanda 7): el mecanismo en negativo, los 32 pares quedan 0/32 adyacentes y 0/32 en octeto en Mawangdui (distancia media 24,4 vs azar 21,7); la hermandad no se hereda por los pares |
| La pregunta del par (qué ordena cada par) | 🟢 pregunta-del-par | Sí | Micro-teorema: "más yang primero" empata 28/28; ningún criterio binario se aparta de una moneda. Ampliación (tanda 7): la propuesta nuclear publicada no se confirma (mayor hu gua 8/24, p = 0,076); diálogo con Radisic (2026), cuya partición de los pares coincide con la nuestra. Sello reevaluable |
| Puente soberanos ↔ palacios | 🟢 calendario-soberanos (sección) | Sí | Teorema verificado: los 12 monótonos son las primeras 6 generaciones del palacio de Qian y las 6 del de Kun (6 y 6, cero en los otros); la observación cualitativa está en Yijing Dao, aquí se demuestra |
