...
**[matchings2026]** Radisic, A. (2026). *Optimal Equivariant Matchings on the 6-Cube with an Application to the King Wen Sequence* [Preprint]. arXiv. https://arxiv.org/abs/2601.07175
Verificación: página de arXiv consultada en línea el 27 de julio de 2026 (arXiv:2601.07175 [math.GM], autor Alejandro Radisic, enviado el 12 de enero de 2026). Evidencia de soporte: estudia los emparejamientos perfectos del hipercubo booleano de 6 dimensiones bajo las simetrías generadas por el complemento y la reversión de bits, demuestra que existe un minimizador único del costo de Hamming (regla de prioridad por reversión, costo 120) y que la secuencia del Rey Wen corresponde a ese emparejamiento óptimo cuando se priorizan las restricciones de conservación de peso. Fundamenta el diálogo del experimento 43 (la partición de los 32 pares en 4 por opuesto a distancia 6, 4 anti-simétricos y 24 por volteo a distancia 2 o 4, y su respeto de las órbitas de Klein, que la suite ya asierta en verificar_simetrias y verificar_rey_wen), tipo 3 (dos análisis independientes que coinciden).
...
**Bloque hermandad, el mecanismo en negativo** (exp. 42, ampliación de la tanda 7)
- "Los 32 pares del Rey Wen quedan 0/32 adyacentes en Mawangdui y 0/32 dentro del mismo octeto, con distancia media de posiciones 24,4 (azar 21,7)" → tipo 1 → suite (verificar_hermandad). Explicación estructural: el volteo cambia el trigrama superior casi siempre, así que la organización por octetos de Mawangdui (por trigrama superior) separa a los pares por construcción. La hermandad no se hereda por los pares; su mecanismo queda como pregunta abierta.

**Bloque los diálogos del par** (exp. 43, ampliación de la tanda 7)
- "La propuesta publicada de que el hexagrama nuclear decide la orientación del par no se confirma: el criterio de poner primero al de mayor hu gua da 8 de 24 decidibles (4 empates), es decir 16/24 hacia el menor primero, p = 0,076, la señal más fuerte de la batería, sugerente y no significativa" → tipo 1 → suite (verificar_pregunta_par). Dato acompañante: en 16 de los 28 pares, los dos miembros colapsan a núcleos distintos del cuarto nivel del bosque nuclear (hu gua iterado).
- "Radisic (2026) formaliza la partición de los 32 pares (4 por opuesto a distancia 6, 4 anti-simétricos, 24 por volteo a distancia 2 o 4) y su respeto de las órbitas de Klein; nuestra suite ya asierta esas mismas particiones y órbitas" → tipo 3 → [matchings2026]. Dos análisis independientes que coinciden (verificar_simetrias, verificar_rey_wen, verificar_pregunta_par). Contexto histórico nombrado sin adoptar el marco: McKenna estudió en los años 80 el perfil de diferencias del Rey Wen dentro de su Timewave, que el registro clasifica fuera del criterio.

**Bloque puente soberanos-palacios** (exp. 28, sección nueva de la tanda 7)
- "Los 12 hexagramas soberanos (monótonos de Q6) son exactamente las primeras seis generaciones del palacio de Qian y las primeras seis del de Kun (6 y 6; cero en los otros seis palacios)" → tipo 1 → suite (verificar_soberanos). Crédito en el copy: la observación cualitativa está en Yijing Dao; aquí se demuestra y se asierta.
...
Añadida en la tanda 7: [matchings2026] (Radisic, 2026), preprint de arXiv verificado en línea el 27 de julio de 2026, para el diálogo del experimento 43. Pendientes de ficha formal, nombrados solo como contexto en el copy: Han Zhongmin (estudio comparativo Mawangdui-Rey Wen, en chino), iching-math (recurso web con el tipo de ciclo [52, 10, 2]) y McKenna (Timewave, clasificado fuera del criterio en el registro); no se citan como respaldo.
